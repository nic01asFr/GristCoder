...
EXAMPLE = AssistantManifest(
    ...
    playbooks=[
        Playbook(id="cotation_guidee", title="Cotation guidée par le référentiel",
                 ...
                 steps=["Résoudre la ligne Entites cible (objet_id + niveau) -> ce sera Cotations.objet",
                        "Lire Referentiel_items filtré par Entites.niveau + condition d'applicabilité (grammaire = / in() / <> N, fallback True)",
                        "Pour chaque item applicable : rappeler definition + lib_1..4, proposer une note justifiée",
                        "Gérer sans objet (999) et note_potentiel si potentiel_pertinent",
                        "Écrire Cotations via grist_apply (applyUserActions) : {objet, item(Text)+brique, note, note_potentiel, potentiel_commentaire, profondeur (jamais vide), prov, statut='propose', confiance, date_cotation, auteur}. NE PAS écrire niveau/applicable/gristHelper_Display (formules)."]),
