...
EXAMPLE = AssistantManifest(
    ...
    bindings=[
        Binding(table="Referentiel_items", role="catalog", note="catalogue d'items à coter (source d'applicabilité)"),
        Binding(table="Entites", role="data", note="objets cotés (niveau site/assise/batiment/zone)"),
        Binding(table="Cotations", role="data", note="notes portées (écriture bornée ici)"),
        Binding(table="Batiments", role="reference", note="inventaire, pivot rnb, attributs d'applicabilité"),
        Binding(table="Sites", role="reference", note="unité foncière, OPERAT"),
        Binding(table="Factures", role="energy", note="OCR VLM -> réconciliation énergie"),
